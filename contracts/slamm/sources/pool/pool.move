...
module slamm::pool {
    ...
    public fun deposit_liquidity<A, B, Hook: drop, State: store, P>(
        ...
        let initial_total_funds_a = pool.total_funds_a();
        let initial_total_funds_b = pool.total_funds_b();

        let balance_a = coin_a.balance_mut().split(quote.deposit_a());
        let balance_b = coin_b.balance_mut().split(quote.deposit_b());

        // Add liquidity to pool
        pool.total_funds_a.deposit(bank_a, balance_a);
        pool.total_funds_b.deposit(bank_b, balance_b);
    ...
    public fun redeem_liquidity<A, B, Hook: drop, State: store, P>(
        ...
        let initial_lp_supply = pool.lp_supply.supply_value();
        let initial_reserve_a = pool.total_funds_a();
        ...
        let mut balance_a = pool.total_funds_a.withdraw(bank_a, quote.withdraw_a());
        let mut balance_b = pool.total_funds_b.withdraw(bank_b, quote.withdraw_b());

        // let (fee_amount_a, fee_amount_b) = pool.compute_redemption_fees_(balance_a.value(), balance_b.value());
    ...
    public fun total_funds<A, B, Hook: drop, State: store>(pool: &Pool<A, B, Hook, State>): (u64, u64) {
        (
            pool.total_funds_a(),
            pool.total_funds_b(),
        )
    ...
    public fun total_funds_a<A, B, Hook: drop, State: store>(pool: &Pool<A, B, Hook, State>): u64 {
        pool.total_funds_a.0
    }
    
    public fun total_funds_b<A, B, Hook: drop, State: store>(pool: &Pool<A, B, Hook, State>): u64 {
        pool.total_funds_b.0
    }
    
    public fun protocol_fees<A, B, Hook: drop, State: store>(pool: &Pool<A, B, Hook, State>): &Fees<A, B> {
        &pool.protocol_fees
    }
    
    public fun pool_fee_config<A, B, Hook: drop, State: store>(pool: &Pool<A, B, Hook, State>): &FeeConfig {
        &pool.pool_fee_config
    }
    
    public fun lp_supply_val<A, B, Hook: drop, State: store>(pool: &Pool<A, B, Hook, State>): u64 {
        pool.lp_supply.supply_value()
    }
    
    public fun trading_data<A, B, Hook: drop, State: store>(pool: &Pool<A, B, Hook, State>): &TradingData {
        &pool.trading_data
    }
    ...
    public fun total_swap_a_in_amount(trade_data: &TradingData): u128 { trade_data.swap_a_in_amount }
    public fun total_swap_b_out_amount(trade_data: &TradingData): u128 { trade_data.swap_b_out_amount }
    public fun total_swap_a_out_amount(trade_data: &TradingData): u128 { trade_data.swap_a_out_amount }
    public fun total_swap_b_in_amount(trade_data: &TradingData): u128 { trade_data.swap_b_in_amount }
    public fun protocol_fees_a(trade_data: &TradingData): u64 { trade_data.protocol_fees_a }
    public fun protocol_fees_b(trade_data: &TradingData): u64 { trade_data.protocol_fees_b }
    public fun pool_fees_a(trade_data: &TradingData): u64 { trade_data.pool_fees_a }
    public fun pool_fees_b(trade_data: &TradingData): u64 { trade_data.pool_fees_b }

    public fun minimum_liquidity(): u64 { MINIMUM_LIQUIDITY }
    public fun intent_quote<A, B, Hook: drop, State: store>(intent: &Intent<A, B, Hook, State>): &SwapQuote { &intent.quote }
    ...
    public(package) fun compute_swap_fees_<A, B, Hook: drop, State: store>(pool: &Pool<A, B, Hook, State>, amount: u64): (u64, u64) {
    ...
    public fun swap_result_user(result: &SwapResult): address { result.user }
    public fun swap_result_pool_id(result: &SwapResult): ID { result.pool_id }
    public fun swap_result_amount_in(result: &SwapResult): u64 { result.amount_in }
    public fun swap_result_amount_out(result: &SwapResult): u64 { result.amount_out }

    public fun swap_result_protocol_fees(result: &SwapResult): u64 {
        result.output_fees.protocol_fees()
    }

    public fun swap_result_pool_fees(result: &SwapResult): u64 {
        result.output_fees.pool_fees()
    }

    public fun swap_result_a2b(result: &SwapResult): bool { result.a2b }

    public fun deposit_result_user(result: &DepositResult): address { result.user }
    public fun deposit_result_pool_id(result: &DepositResult): ID { result.pool_id }
    public fun deposit_result_deposit_a(result: &DepositResult): u64 { result.deposit_a }
    public fun deposit_result_deposit_b(result: &DepositResult): u64 { result.deposit_b }
    public fun deposit_result_mint_lp(result: &DepositResult): u64 { result.mint_lp }

    public fun redeem_result_user(result: &RedeemResult): address { result.user }
    public fun redeem_result_pool_id(result: &RedeemResult): ID { result.pool_id }
    public fun redeem_result_withdraw_a(result: &RedeemResult): u64 { result.withdraw_a }
    public fun redeem_result_withdraw_b(result: &RedeemResult): u64 { result.withdraw_a }
    public fun redeem_result_burn_lp(result: &RedeemResult): u64 { result.burn_lp }
    ...
    public(package) fun mut_reserve_a<A, B, Hook: drop, State: store, P>(
        ...
        if (increase) {
            pool.total_funds_a.deposit(bank, balance::create_for_testing(amount));
        } else {
            balance::destroy_for_testing(pool.total_funds_a.withdraw(bank, amount));
        };
    }
    ...
    public(package) fun mut_reserve_b<A, B, Hook: drop, State: store, P>(
        ...
        if (increase) {
            pool.total_funds_b.deposit(bank, balance::create_for_testing(amount));
        } else {
            balance::destroy_for_testing(pool.total_funds_b.withdraw(bank, amount));
        };
    }
