...
use steamm_scripts::pool_script::{multi_swap_quote, MultiRouteSwapQuote};
...
public fun deposit_liquidity<P, A, B, BTokenA, BTokenB, Quoter: store, LpType: drop>(
    ...
    coin_a.join(bank_a.burn_btoken(lending_market, btoken_a, clock, ctx));
    coin_b.join(bank_b.burn_btoken(lending_market, btoken_b, clock, ctx));
...
public fun redeem_liquidity<P, A, B, BTokenA, BTokenB, Quoter: store, LpType: drop>(
    ...
    let (btoken_a, btoken_b, _) = pool.redeem_liquidity(lp_tokens, min_a, min_b, ctx);
...
public fun to_multi_swap_route<P, X, Y, BTokenX, BTokenY>(
    ...
    let quote = multi_swap_quote(amount_in, amount_out);
