...
fun test_minimum_bank_tokens() {
    ...
    let val_a = btoken_a.value();
    let val_b = btoken_b.value();
    let coin_a = bank_a.burn_btokens(&mut lending_market, &mut btoken_a, val_a, &clock, ctx);
    let coin_b = bank_b.burn_btokens(&mut lending_market, &mut btoken_b, val_b, &clock, ctx);
