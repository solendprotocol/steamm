...
module steamm::stable {
    ...
    public struct StableQuoter<phantom W> has store {
        min_fee_bps: u64,
        max_fee_bps: u64,
        version: Version,
    }
    ...
    public fun new<A, B, W: drop, P>(
        ...
        swap_fee_bps: u64,
        ctx: &mut TxContext,
    ): (Pool<A, B, StableQuoter<W>, P>, PoolCap<A, B, StableQuoter<W>, P>) {
        assert!(max_fee_bps <= BPS_DENOMINATOR, EFeeAbove100Percent);
        assert!(min_fee_bps < max_fee_bps, EMaxFeeMustBeBiggerThenMinFee);

        let inner = StableQuoter<W> {
            version: version::new(CURRENT_VERSION),
            min_fee_bps,
            max_fee_bps,
        };

        let (pool, pool_cap) = pool::new<A, B, StableQuoter<W>, P>(
            registry,
            swap_fee_bps,
            inner,
            ctx,
    ...
    public fun intent_swap<A, B, W: drop, P>(
        self: &mut Pool<A, B, StableQuoter<W>, P>,
        amount_in: u64,
        a2b: bool,
    ): Intent<A, B, StableQuoter<W>, P> {
    ...
    public fun execute_swap<A, B, W: drop, P>(
        self: &mut Pool<A, B, StableQuoter<W>, P>,
        intent: Intent<A, B, StableQuoter<W>, P>,
    ...
    public fun quote_swap<A, B, W: drop, P>(
        self: &Pool<A, B, StableQuoter<W>, P>,
        amount_in: u64,
        a2b: bool,
    ): SwapQuote {
        ...
        let final_funds_a = if (a2b) {
            self.btoken_amount_a() + amount_in
        } else {
            self.btoken_amount_a() - amount_out
        };
        
        let final_funds_b = if (a2b) {
            self.btoken_amount_b() - amount_out
        } else {
            self.btoken_amount_b() + amount_in
        };
        ...
        let variable_fee_bps = compute_fee(
            ...
            final_reserve_ratio,
            self.quoter().min_fee_bps,
            self.quoter().max_fee_bps,
        );
    ...
    public fun reserve_ratio<A, B, W: drop, P>(
        self: &Pool<A, B, StableQuoter<W>, P>,
    ): Decimal {
        reserve_ratio_(
    ...
    public fun k<A, B, Hook: store, P>(self: &Pool<A, B, Hook, P>): u128 {
        let (btokens_a, btokens_b) = self.btoken_amounts();
        ((btokens_a as u128) + (btokens_b as u128))
    }
    ...
    entry fun migrate<A, B, W, P>(
        self: &mut Pool<A, B, StableQuoter<W>, P>,
        _cap: &PoolCap<A, B, StableQuoter<W>, P>,
    ) {
        migrate_(self);
    }
    ...
    public(package) fun assert_invariant_does_not_decrease<A, B, Hook: store, P>(self: &Pool<A, B, Hook, P>, k0: u128) {
        let k1 = k(self);
