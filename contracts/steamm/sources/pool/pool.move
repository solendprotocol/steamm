...
module steamm::pool {
    ...
    public fun deposit_liquidity<A, B, Quoter: store, P>(
        ...
        let (initial_total_funds_a, initial_total_funds_b) = pool.btoken_amounts();

        let balance_a = coin_a.balance_mut().split(quote.deposit_a());
        let balance_b = coin_b.balance_mut().split(quote.deposit_b());

        // Add liquidity to pool
        pool.btokens_a.join(balance_a);
        pool.btokens_b.join(balance_b);
    ...
    public fun redeem_liquidity<A, B, Quoter: store, P>(
        ...
        let initial_lp_supply = pool.lp_supply.supply_value();
        let initial_reserve_a = pool.btoken_amount_a();
        ...
        // Withdraw
        let mut balance_a = pool.btokens_a.split(quote.withdraw_a());
        let mut balance_b = pool.btokens_b.split(quote.withdraw_b());
    ...
    public fun btoken_amounts<A, B, Quoter: store, P>(pool: &Pool<A, B, Quoter, P>): (u64, u64) {
        (
            pool.btoken_amount_a(),
            pool.btoken_amount_b(),
        )
    ...
    public fun btoken_amount_a<A, B, Quoter: store, P>(pool: &Pool<A, B, Quoter, P>): u64 {
        pool.btokens_a.value()
    }
    
    public fun btoken_amount_b<A, B, Quoter: store, P>(pool: &Pool<A, B, Quoter, P>): u64 {
        pool.btokens_b.value()
    }
    
    public fun protocol_fees<A, B, Quoter: store, P>(pool: &Pool<A, B, Quoter, P>): &Fees<BToken<P, A>, BToken<P, B>> {
        &pool.protocol_fees
    }
    
    public fun pool_fee_config<A, B, Quoter: store, P>(pool: &Pool<A, B, Quoter, P>): &FeeConfig {
        &pool.pool_fee_config
    }
    
    public fun lp_supply_val<A, B, Quoter: store, P>(pool: &Pool<A, B, Quoter, P>): u64 {
        pool.lp_supply.supply_value()
    }
    
    public fun trading_data<A, B, Quoter: store, P>(pool: &Pool<A, B, Quoter, P>): &TradingData {
        &pool.trading_data
    }
    ...
    public fun intent_quote<A, B, Quoter: store, P>(pool: &Intent<A, B, Quoter, P>): &SwapQuote { &pool.quote }
    ...
    public(package) fun compute_swap_fees_<A, B, Quoter: store, P>(pool: &Pool<A, B, Quoter, P>, amount: u64): (u64, u64) {
    ...
    public(package) fun mut_reserve_a<A, B, Quoter: store, P>(
        ...
        if (increase) {
            pool.btokens_a.join(balance::create_for_testing(amount));
        } else {
            balance::destroy_for_testing(pool.btokens_a.split(amount));
        };
    }
    ...
    public(package) fun mut_reserve_b<A, B, Quoter: store, P>(
        ...
        if (increase) {
            pool.btokens_b.join(balance::create_for_testing(amount));
        } else {
            balance::destroy_for_testing(pool.btokens_b.split(amount));
        };
    }
