...
export function isVerifiedIssuer(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V26}::zklogin_verified_issuer::VerifiedIssuer`; }
...
export class VerifiedIssuer implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `${PKG_V26}::zklogin_verified_issuer::VerifiedIssuer`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = VerifiedIssuer.$typeName; readonly $fullTypeName: `${typeof PKG_V26}::zklogin_verified_issuer::VerifiedIssuer`; readonly $typeArgs: []; readonly $isPhantom = VerifiedIssuer.$isPhantom;

 readonly id: ToField<UID>; readonly owner: ToField<"address">; readonly issuer: ToField<String>

 private constructor(typeArgs: [], fields: VerifiedIssuerFields, ) { this.$fullTypeName = composeSuiType( VerifiedIssuer.$typeName, ...typeArgs ) as `${typeof PKG_V26}::zklogin_verified_issuer::VerifiedIssuer`; this.$typeArgs = typeArgs;

 this.id = fields.id;; this.owner = fields.owner;; this.issuer = fields.issuer; }

 static reified( ): VerifiedIssuerReified { return { typeName: VerifiedIssuer.$typeName, fullTypeName: composeSuiType( VerifiedIssuer.$typeName, ...[] ) as `${typeof PKG_V26}::zklogin_verified_issuer::VerifiedIssuer`, typeArgs: [ ] as [], isPhantom: VerifiedIssuer.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => VerifiedIssuer.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => VerifiedIssuer.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => VerifiedIssuer.fromBcs( data, ), bcs: VerifiedIssuer.bcs, fromJSONField: (field: any) => VerifiedIssuer.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => VerifiedIssuer.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => VerifiedIssuer.fromSuiParsedData( content, ), fromSuiObjectData: (content: SuiObjectData) => VerifiedIssuer.fromSuiObjectData( content, ), fetch: async (client: SuiClient, id: string) => VerifiedIssuer.fetch( client, id, ), new: ( fields: VerifiedIssuerFields, ) => { return new VerifiedIssuer( [], fields ) }, kind: "StructClassReified", } }
