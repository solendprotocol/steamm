...
export function migrate( tx: Transaction, typeArgs: [string, string, string], args: MigrateArgs ) { return tx.moveCall({ target: `${PUBLISHED_AT}::cpmm::migrate`, typeArguments: typeArgs, arguments: [ obj(tx, args.pool), obj(tx, args.cap) ], }) }

export function migrate_( tx: Transaction, typeArgs: [string, string, string], pool: TransactionObjectInput ) { return tx.moveCall({ target: `${PUBLISHED_AT}::cpmm::migrate_`, typeArguments: typeArgs, arguments: [ obj(tx, pool) ], }) }

export interface MigrateAsGlobalAdminArgs { pool: TransactionObjectInput; admin: TransactionObjectInput }

export function migrateAsGlobalAdmin( tx: Transaction, typeArgs: [string, string, string], args: MigrateAsGlobalAdminArgs ) { return tx.moveCall({ target: `${PUBLISHED_AT}::cpmm::migrate_as_global_admin`, typeArguments: typeArgs, arguments: [ obj(tx, args.pool), obj(tx, args.admin) ], }) }
...
export interface ExecuteSwapArgs { pool: TransactionObjectInput; bankA: TransactionObjectInput; bankB: TransactionObjectInput; intent: TransactionObjectInput; coinA: TransactionObjectInput; coinB: TransactionObjectInput; minAmountOut: bigint | TransactionArgument }

export function executeSwap( tx: Transaction, typeArgs: [string, string, string, string], args: ExecuteSwapArgs ) { return tx.moveCall({ target: `${PUBLISHED_AT}::cpmm::execute_swap`, typeArguments: typeArgs, arguments: [ obj(tx, args.pool), obj(tx, args.bankA), obj(tx, args.bankB), obj(tx, args.intent), obj(tx, args.coinA), obj(tx, args.coinB), pure(tx, args.minAmountOut, `u64`) ], }) }
...
export function intentSwap( tx: Transaction, typeArgs: [string, string, string], args: IntentSwapArgs ) { return tx.moveCall({ target: `${PUBLISHED_AT}::cpmm::intent_swap`, typeArguments: typeArgs, arguments: [ obj(tx, args.pool), pure(tx, args.amountIn, `u64`), pure(tx, args.a2B, `bool`) ], }) }
